...
class GfcControllerImplTest {
    ...
    void shouldListMethodsFromUploadedJavaFileThroughInputPort() {
        MockMultipartFile file = new MockMultipartFile(
                ...
                "Exemplo.java",
                "text/plain",
                "class Exemplo { int soma(int a, int b) { return a + b; } }".getBytes()
        );
        ...
        ResponseEntity<List<GfcSourceMethodDTO>> response = controller.methods(file);
        ...
        assertEquals("class Exemplo { int soma(int a, int b) { return a + b; } }", inputCaptor.getValue().sourceCode());
    ...
    void shouldRejectNonJavaFileUpload() {
        ...
        org.junit.jupiter.api.Assertions.assertThrows(JavaSourceFileException.class, () -> controller.methods(file));
    }
}

...
export const ReturnNode = memo(function ReturnNode({ data, selected }: NodeProps<GFCFlowNode>) {
  ...
  return (
    <div
      ...
      style={{
        background: 'var(--color-gfc-return-bg)',
        boxShadow: selected
          ? `inset 0 0 0 2px ${borderColor}, 0 0 16px rgba(248, 81, 73, 0.4), 0 4px 10px rgba(0,0,0,0.3)`
          : `inset 0 0 0 2px ${borderColor}, 0 4px 10px rgba(0,0,0,0.3)`,
      }}
    >
      <Handle id="top" type="target" position={Position.Top} className="w-2.5! h-2.5!" style={{ background: 'var(--color-gfc-return)' }} />
      ...
      <div className="flex items-center gap-2">
        ...
        {startLine != null && (
          <span className="text-[9px] font-mono text-white/70 shrink-0">L{startLine}</span>
        )}
      </div>
      ...
      <Handle id="bottom" type="source" position={Position.Bottom} className="w-2.5! h-2.5!" style={{ background: 'var(--color-gfc-return)' }} />
    </div>
  );
});

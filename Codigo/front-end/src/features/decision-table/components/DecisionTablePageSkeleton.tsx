...
interface DecisionTablePageSkeletonProps {
  projectName: string;
  projectId: string;
}

/**
 * Esqueleto da `DecisionTablePage`. Reproduz toolbar + painel de
 * condições/ações + matriz + status bar pra evitar tela em branco no load.
 */
export function DecisionTablePageSkeleton({ projectName, projectId }: DecisionTablePageSkeletonProps) {
  return (
    <div className="h-screen flex flex-col bg-surface">
      ...
      <div className="h-12 bg-surface-card border-b border-edge flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="h-3.5 w-44 rounded bg-surface-hover animate-pulse" />
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <aside className="w-56 border-r border-edge bg-surface shrink-0 flex flex-col p-3 gap-3">
          <div className="h-3.5 w-32 rounded bg-surface-hover animate-pulse" />
          <div className="space-y-2 mt-1">
            {[0, 1, 2].map((i) => (
              <div
                key={`c-${i}`}
                className="h-9 w-full rounded bg-surface-hover animate-pulse"
                style={{ animationDelay: `${i * 80}ms` }}
              />
            ))}
          </div>
          <div className="h-3.5 w-24 rounded bg-surface-hover animate-pulse mt-3" />
          <div className="space-y-2 mt-1">
            {[0, 1].map((i) => (
              <div
                key={`e-${i}`}
        ...
        <div className="flex-1 relative bg-surface flex items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            <div className="relative w-16 h-16 flex items-center justify-center">
              ...
              <Table className="w-7 h-7 text-primary-light relative z-10" />
            </div>
            <div className="flex flex-col items-center gap-1.5">
              <p className="text-sm text-gray-300 font-medium">Carregando tabela de decisão</p>
              <p className="text-xs text-gray-500">Sincronizando regras com o GCE…</p>
            </div>
          </div>
        </div>
      </div>

      <div className="h-8 bg-surface-card border-t border-edge flex items-center px-4 gap-4 shrink-0">

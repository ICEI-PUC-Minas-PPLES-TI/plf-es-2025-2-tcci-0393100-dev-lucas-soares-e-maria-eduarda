...
export const OperatorNode = memo(function OperatorNode({ data, selected }: NodeProps<GCEFlowNode>) {
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Left} className="w-2! h-2! bg-node-operator!" />
      ...
      <Handle type="source" position={Position.Right} className="w-2! h-2! bg-node-operator!" />
    </div>
  );
});
